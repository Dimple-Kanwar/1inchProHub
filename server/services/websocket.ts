...
class WebSocketService {
  ...
  private clients: Map<WebSocket, SubscriptionData> = new Map();
  ...
  private handleUnsubscription(ws: WebSocket, data: any) {
    ...
    if (clientData) {
      ...
      if (data.tokens) {
        clientData.tokens = clientData.tokens?.filter(token => !data.tokens.includes(token));
      }
      if (data.pairs) {
        clientData.pairs = clientData.pairs?.filter(pair => !data.pairs.includes(pair));
      }
  ...
  private broadcast(type: string, data: any, filter?: (clientData: SubscriptionData) => boolean) {
      ...
      try {
        ...
        if (allTokens.size > 0) {
          ...
          this.broadcast('price_update', {
            ...
            tokens,
            timestamp: Date.now()
          }, (clientData) => Boolean(clientData.tokens?.some(token => tokens.includes(token))));
        }
      } catch (error) {
  ...
  async sendSwapUpdate(userId: string, swapData: any) {
    this.broadcast('swap_update', swapData, (clientData) => clientData.userId === userId);
  }

  async sendStrategyUpdate(userId: string, strategyData: any) {
    this.broadcast('strategy_update', strategyData, (clientData) => clientData.userId === userId);
  }

  async sendPortfolioUpdate(userId: string, portfolioData: any) {
    this.broadcast('portfolio_update', portfolioData, (clientData) => clientData.userId === userId);
  }

  async sendNotification(userId: string, notification: any) {
    this.broadcast('notification', notification, (clientData) => clientData.userId === userId);
  }
